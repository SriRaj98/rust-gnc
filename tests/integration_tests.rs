...
fn test_safety_disarm_isolation() {
    ...
    let outputs = stabilizer.tick(input, input, input, target, 0.5, 0.01);
...
fn test_disarm_safety_guarantee() {
    ...
    let motors = stabilizer.tick(input, input, input, target, 1.0, 0.01);
...
fn test_state_reset_on_rearm() {
    ...
    for _ in 0..100 {
        stabilizer.tick(error, error, error, Attitude::default(), 0.5, 0.01);
    }
    ...
    stabilizer.set_armed(ArmingState::Armed);
    let motors = stabilizer.tick(error, error, error, Attitude::default(), 0.5, 0.01);
